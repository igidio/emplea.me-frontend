<template>
	...
	>
		...
		<span class="error text-right" v-if="error">{{error.message }}</span>
...
import {employerSocialCreate, seekerSocialCreate} from "~/queries";
...
const {mutate, error, loading} = useMutation(employerSocialCreate)

const submit = async () => {
	await mutate({
		"createEmployerSocialInput": {
