...
import {gqlPostSkill, seekerFindAny, seekerSkillDelete, seekerSkillUpdate} from "~/queries";
...
const {
	...
	loading,
	error
} = useMutation<{ postSkillUpdate: string }>(gqlPostSkill.update)
...
const submit = async () => {
	await update({
		...
		useToast().add({title: e?.data?.postSkillUpdate})
		is_editable.value = false;
	}).catch((e: Error) => {
		console.log(e)
	})
...
} = useMutation<{ postSkillDelete: string }>(gqlPostSkill.delete)

const delete_skill = async () => {
	await delete_skill_mutation({
		"postSkillDeleteId": Number(p.props.id),
		"postId": +(useRoute().params.id!)
	}).then(async (r) => {
		await p.props.reload()
		useToast().add({title: r?.data?.postSkillDelete})
	}).catch((e: Error) => {
		console.log(e)
	})
}
